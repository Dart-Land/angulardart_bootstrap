...
bool _isStaticPositioned(Element nativeEl) => or(nativeEl.style.position, "static") == "static";
...
_parentOffsetEl(Element nativeEl) {
  ...
  while (offsetParent != null && offsetParent != _document && _isStaticPositioned(offsetParent)) {
...
Rectangle _position(Element el) {
  ...
  return new Rectangle(elOffset.left, elOffset.top, boundingClientRect.width ?? el.offsetWidth, boundingClientRect.height ?? el.offsetHeight);
}

/// Provides coordinates for the targetEl in relation to hostEl
Position positionElements(Element hostEl, Element targetEl, String positionStr, bool appendToBody) {
  ...
  switch (pos0) {
    case "right":
      targetElPos = new Position(top: shiftHeight[pos1](), left: shiftWidth[pos0]());
      break;
    case "left":
      targetElPos = new Position(top: shiftHeight[pos1](), left: hostElPos.left - targetElWidth);
      break;
    case "bottom":
      targetElPos = new Position(top: shiftHeight[pos0](), left: shiftWidth[pos1]());
      break;
    default:
      targetElPos = new Position(top: hostElPos.top - targetElHeight, left: shiftWidth[pos1]());
  }
  return targetElPos;
}
